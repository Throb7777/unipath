...
- 先给当前服务起一个清楚的名字，例如 `家中转发服务`、`私网转发服务`
- 点击 **保存** 后，会保存或更新当前服务档
- 回到 **首页** 后，可以通过 **服务** 入口切换或删除已保存服务
...
- 当连接类型是 **Private Network** 时，认证 Token 输入框会直接显示
- 当连接类型是 **Local Network** 时，认证 Token 默认收起，需要时再展开
- 模式列表现在默认只突出显示模式名称；当前选中模式的说明会显示在列表下方
